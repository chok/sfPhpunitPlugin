...
class sfPhpunitCreateModelsTestTask extends sfBasePhpunitCreateTask
{
	...
	protected function configure()
	{
      ...
      $this->addOptions(array(
      	new sfCommandOption('type', null, sfCommandOption::PARAMETER_REQUIRED , 'Model type (propel,doctrine)', null),
...
EOF;
	...
	{
	  $this->setType($this->checkORMType($options['type']));
      ...
      {
      	$options['class_path'] = $this->modelTypes[$this->getType()]['default_model_path'];
      }
      ...
      foreach($paths as $path)
      {
      	...
      	$ignoredDirs= $this->modelTypes[$this->getType()]['ignored_directory'];
      ...
      foreach ($paths as $path)
      {
      	...
      	while ($dir->valid())
      	{
      		if (strpos($dir, '.php') !== false)
      		{
      			...
      			$suffix = !empty($options['file_suffix'])? $options['file_suffix'] : $this->modelTypes[$this->getType()]['classFileSuffix'];
	...
	private function createTestClass($arguments, $subfolder = null)
	{
		...
		$suffix = !empty($arguments['file_suffix'])? $arguments['file_suffix'] : $this->modelTypes[$this->getType()]['classFileSuffix'];
  ...
  public function getORMType($type)
  {
    if (is_null($type))
    {
